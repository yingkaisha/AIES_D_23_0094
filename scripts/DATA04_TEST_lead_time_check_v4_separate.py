...
# import filenames
filenames = np.load('/glade/work/ksha/NCAR/filenames_for_TEST_inds.npy', allow_pickle=True)[()]
...
for lead1_base in range(2, 21):
    ...
    filename_test_lead2 = filenames['lead{}'.format(lead1_base)]
    filename_test_lead3 = filenames['lead{}'.format(lead2_base)]
    filename_test_lead4 = filenames['lead{}'.format(lead3_base)]
    filename_test_lead5 = filenames['lead{}'.format(lead4_base)]
    
    indx2, indy2, lead2, day2 = id_extract(filename_test_lead2)
    indx3, indy3, lead3, day3 = id_extract(filename_test_lead3)
    indx4, indy4, lead4, day4 = id_extract(filename_test_lead4)
    indx5, indy5, lead5, day5 = id_extract(filename_test_lead5)

    L = len(filename_test_lead2)
    ...
    for i in picks:
        ...
        for i3, name3 in enumerate(filename_test_lead3[i_start:i_end]):
            if (pattern_day in name3) and (patten_lead3 in name3):
                ind_temp3 = i_start+i3
                break;

        for i4, name4 in enumerate(filename_test_lead4[i_start:i_end]):
            if (pattern_day in name4) and (patten_lead4 in name4):
                ind_temp4 = i_start+i4
                break;

        for i5, name5 in enumerate(filename_test_lead5[i_start:i_end]):
    ...
    np.save('/glade/work/ksha/NCAR/TEST_lead_inds_{}{}{}{}_part{}.npy'.format(lead1_base, lead2_base, lead3_base, lead4_base, part), save_dict)
    print('/glade/work/ksha/NCAR/TEST_lead_inds_{}{}{}{}_part{}.npy'.format(lead1_base, lead2_base, lead3_base, lead4_base, part))
